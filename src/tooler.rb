module Tooler
  ...
  def self.configure(config, ansible_dir, scripts_dir, conf_dir)
    ...
    Base.configure(config, base_file, scripts_dir, ansible_dir) if Base.validate(base_file, scripts_dir, ansible_dir)
    Tools.configure(config, tools_dir) if Tools.validate(tools_dir)
  end
end
